...
(alpha=0x0109 / beta=0x8003, with masks 0x6A74 / 0x6AFC) passes
...
Usage:
    python compare_bct_key_recovery_0109.py          # 7-round (default)
    python compare_bct_key_recovery_0109.py --rounds 6
"""
...
# Identical to plot_truncated_bct_self_0109.py
WORD_SIZE = 16
...
np.save(os.path.join(OUTDIR, f'data_wrong_key_mean_{NR}r_bct_0109.npy'), m_bct.astype(np.float32))
np.save(os.path.join(OUTDIR, f'data_wrong_key_std_{NR}r_bct_0109.npy'),  s_bct_orig_std.astype(np.float32))
...
plt.tight_layout()
fig_path = os.path.join(OUTDIR, f'bct_profile_compare_{NR}r_0109.png')
...
plt.tight_layout()
rank_fig_path = os.path.join(OUTDIR, f'bct_key_rank_compare_{NR}r_0109.png')
...
if args.no_keras:
    ...
    try:
        ...
        np.save(os.path.join(OUTDIR, f'net_ranks_orig_{NR}r_0109.npy'), net_ranks_orig)
        np.save(os.path.join(OUTDIR, f'net_ranks_bct_{NR}r_0109.npy'),  net_ranks_bct)
